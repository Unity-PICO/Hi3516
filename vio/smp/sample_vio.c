...
HI_S32 set_sensor_saturation_zero(VI_PIPE ViPipe)
{
    HI_S32             s32Ret = HI_SUCCESS;
    ISP_SATURATION_ATTR_S stSaturationAttr;		/* isp saturation attr */
...
HI_S32 SAMPLE_VIO_ViOnlineVpssOnlineRoute(HI_U32 u32VoIntfType)
{
    ...
    stViConfig.astViInfo[1].stPipeInfo.aPipe[0]       = 2;
...
HI_S32 SAMPLE_VIO_VPSS_VO_MIPI_TX(HI_U32 u32VoIntfType, HI_U32 saturation, HI_U32 sns0_rotat,HI_U32 enWDRMode_sel)
{
    HI_S32             s32Ret = HI_SUCCESS;
    ...
    //WDR_MODE_E         enWDRMode      = WDR_MODE_NONE;
	WDR_MODE_E         enWDRMode      = enWDRMode_sel;
    DYNAMIC_RANGE_E    enDynamicRange = DYNAMIC_RANGE_SDR8;
    PIXEL_FORMAT_E     enPixFormat    = PIXEL_FORMAT_YVU_SEMIPLANAR_420;
    VIDEO_FORMAT_E     enVideoFormat  = VIDEO_FORMAT_LINEAR;
    COMPRESS_MODE_E    enCompressMode = COMPRESS_MODE_NONE;
	...
	s32Ret = HI_MPI_VPSS_SetChnRotation (VpssGrp,VpssChn,sns0_rotat);
	if (HI_SUCCESS != s32Ret)
	{
...
HI_S32 SAMPLE_VIO_VPSS_VO_MIPI_TX_DOUBLE_VI(HI_U32 u32VoIntfType, HI_U32 saturation, HI_U32 sns0_rotat, HI_U32 sns1_rotat,HI_U32 enWDRMode_sel)
{
	printf("enWDRMode_sel is %d\n",enWDRMode_sel);
    HI_S32             s32Ret = HI_SUCCESS;
    ...
    astVpssChnAttr[1].u32Width                    = stSize.u32Width;
    astVpssChnAttr[1].u32Height                   = stSize.u32Height;
...
// start vpssgrp 1
	...
	s32Ret = HI_MPI_VPSS_SetChnRotation (VpssGrp[0],VpssChn,sns0_rotat);
	if (HI_SUCCESS != s32Ret)
	{
		SAMPLE_PRT("HI_MPI_VPSS_SetChnRotation sensor1 failed with %d\n", s32Ret);
		goto EXIT1;
	}
	
	s32Ret = HI_MPI_VPSS_SetChnRotation (VpssGrp[1],1,sns1_rotat);
	if (HI_SUCCESS != s32Ret)
	{
		SAMPLE_PRT("HI_MPI_VPSS_SetChnRotation sensor2 failed with %d\n", s32Ret);
		goto EXIT1;
	}
...
HI_S32 SAMPLE_VIO_VPSS_VO_HDMI(HI_U32 u32VoIntfType, HI_U32 saturation, HI_U32 sns0_rotat,HI_U32 enWDRMode_sel)
{
    HI_S32             s32Ret = HI_SUCCESS;
	...
	s32Ret = HI_MPI_VPSS_SetChnRotation(stVoConfig.VoDev, 0, sns0_rotat);
...
HI_S32 SAMPLE_VIO_VPSS_VO_HDMI_DOUBLE_VI(HI_U32 u32VoIntfType, HI_U32 saturation, HI_U32 sns0_rotat, HI_U32 sns1_rotat,HI_U32 enWDRMode_sel)
{
    HI_S32             s32Ret = HI_SUCCESS;
...
//	stVoConfig.enVoPartMode      = 1;
	...
	/* added by rpdzkj to rotate sensor display degrees */  
	s32Ret = HI_MPI_VPSS_SetChnRotation(stVoConfig.VoDev, 0, sns0_rotat);
	if (s32Ret != HI_SUCCESS)                                             
	{                                                            
		printf("Set HI_MPI_VPSS_SetChnRotation failed with errno %#x!\n", s32Ret);  
		return HI_FAILURE;                                                
	}          

	s32Ret = HI_MPI_VPSS_SetChnRotation(stVoConfig.VoDev, 1, sns1_rotat);
	if (s32Ret != HI_SUCCESS)                                             
	{                                                            
		printf("Set HI_MPI_VPSS_SetChnRotation failed with errno %#x!\n", s32Ret);  
		return HI_FAILURE;                                                
