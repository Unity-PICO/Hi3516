...
{
    printf("\n/************************************/\n");
    printf("Usage : %s -l <voIndex>  -f <functionIndex> \n", sPrgNm);
...
HI_S32 SAMPLE_H265_VDEC_VPSS_VO(HI_VOID)
{
    ...
    *************************************************/
    ...
    if(voType == 1) { //MIPI LCD,added by rpdzkj
...
#endif
{
    ...
    while((ch=getopt(argc,argv,"l:f:h")) != -1) {
        // printf("optind:%d\n",optind);
        // printf("optarg:%s\n",optarg);
        // printf("ch:%c\n",ch);
        switch(ch) {
        ...
        case 'f':
            ...
            case 2:
                ...
                break;
            default:
                SAMPLE_PRT("the first index %d is invaild!\n",s32Index);
    ...
    ******************************************/
    ...
    // SAMPLE_PRT("the second index is invaild!\n");
    // SAMPLE_VDEC_Usage(argv[0]);
    // return HI_SUCCESS;
    // }
    /* HI_U32 VO_DEV_TYPE = 0;//HDMI
    ...
    switch (*argv[1])
    {
        case '0':
        {
            s32Ret = SAMPLE_H265_VDEC_VPSS_VO(VO_DEV_TYPE);
            break;
        }
        case '1':
        {
            s32Ret = SAMPLE_H264_VDEC_VPSS_VO(VO_DEV_TYPE);
            break;
        }
        case '2':
        {
            s32Ret = SAMPLE_JPEG_VDEC_VPSS_VO(VO_DEV_TYPE);
            break;
        }
        default :
        {
            SAMPLE_PRT("the first index is invaild!\n");
            SAMPLE_VDEC_Usage(argv[0]);
            s32Ret = HI_FAILURE;
            break;
        }
